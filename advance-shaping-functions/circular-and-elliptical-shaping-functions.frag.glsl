...
//Computational efficiency diminished by the use of a square root
...
//Double-Circle Seat
//Formed by the meeting of two circular arcs, which join with a
//horizontal tangent. The parameter a governs the location of the
...
//Formed by the meeting of two circular arcs, which join with
//a vertical tangent. The parameter a governs the location of
...
//Created by joining of two elliptical arcs that meet at (a, b)
...
//Joins two straight lines with a circular arc whose radius is
//adjustable. User specifics the radius c and the coordinate (a, b)
//in the unit square where the lines would otherwise intersect
...
//Compute the parameters of a circular arc fillet between lines
//L1 (p1 to p2) and L2 (p3 to p4) with radius R
void computeFilletParameters (
    ...
    dP = (a1*a1) + (b1*b1); //Clip or extend lines
    if (dP != 0.0) {
        cP = a1*pCy - b1*pCx;
        pAx = (-a1*c1 - b1*cP)/dP;
        pAy = (a1*cP - b1*c1)/dP;
...
//Passes through a user-specified point in the unit square. The
//point must inhabit a zone close to the identity diagonal.
...
void calcCircleFrom3Points (
    ...
    if (abs(aSlope-bSlope) <= epsilon){
        //check whether the given points are colinear
        return;
    }
